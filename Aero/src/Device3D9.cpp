...
bool Device3D9::render()
{
	...
	if (!Overlay::target_available())
	{
		SetWindowPos(Overlay::m_WndOverlay, HWND_TOPMOST, Overlay::m_tprops.at(0), Overlay::m_tprops.at(1), Overlay::m_tprops.at(2), Overlay::m_tprops.at(3), SWP_HIDEWINDOW);
	}
	else
	{
		SetWindowPos(Overlay::m_WndOverlay, HWND_TOPMOST, Overlay::m_tprops.at(0), Overlay::m_tprops.at(1), Overlay::m_tprops.at(2), Overlay::m_tprops.at(3), SWP_SHOWWINDOW);
	}
	
	
	if (Overlay::target_moved() && Overlay::target_available())
	{
		Sleep(100);
		...
		params.MultiSampleType = D3DMULTISAMPLE_NONE;
		params.PresentationInterval = 0x80000000L;
		if (FAILED(m_Direct3D9Device->Reset(&params))) return false;
	}
