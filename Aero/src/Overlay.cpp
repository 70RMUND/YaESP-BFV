...
bool Overlay::target_available()
{
	auto window_props = get_window_props(m_WndTarget);
	if ((window_props.at(2) == 0) && (window_props.at(3) == 0)) return false;
	return true;
}


bool Overlay::target_moved()
{
	auto window_props = get_window_props(m_WndTarget);

	for (int i=0;i<4;i++)
		if (m_tprops.at(i) != window_props.at(i)) return true;
